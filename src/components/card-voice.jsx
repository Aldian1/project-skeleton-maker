  ...
  return (
    (<div
      ...
      <Card
        ...
        <CardContent
          className={`flex flex-col items-center justify-center space-y-6 ${isRecording ? "text-white" : ""} ${
            isFlipped ? "rotate-y-180" : ""
          } transform-style-3d`}>
          {!isFlipped ? (
            <>
              <div
                className={`relative w-20 h-20 transition-transform duration-500 ${isRecording ? "scale-150" : ""}`}>
                {isRecording ? (
                  <Button
                    variant="ghost"
                    size="lg"
